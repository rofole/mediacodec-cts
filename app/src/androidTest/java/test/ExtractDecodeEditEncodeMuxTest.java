...
//        mOutputFile = sb.toString();
    ...
    private void doExtractDecodeEditEncodeMux(
            ...
            test.OutputSurface outputSurface) {
        ...
        while ((mCopyVideo && !videoEncoderDone) || (mCopyAudio && !audioEncoderDone)) {
            ...
            while (mCopyVideo && !videoDecoderDone
                ...
                if (decoderOutputBufferIndex == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    if (VERBOSE) Log.i(TAG, "no video decoder output buffer");
                    break;
                }
                if (decoderOutputBufferIndex == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
                    if (VERBOSE) Log.i(TAG, "video decoder: output buffers changed");
                    videoDecoderOutputBuffers = videoDecoder.getOutputBuffers();
                    break;
                }
...
//            videoEncoder.setParameters(b);
            ...
            while (mCopyAudio && pendingAudioDecoderOutputBufferIndex != -1) {
                if (VERBOSE) {
                    Log.i(TAG, "audio decoder: attempting to process pending buffer: "
                            + pendingAudioDecoderOutputBufferIndex);
                }
                int encoderInputBufferIndex = audioEncoder.dequeueInputBuffer(TIMEOUT_USEC);
                if (encoderInputBufferIndex == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    if (VERBOSE) Log.i(TAG, "no audio encoder input buffer");
                    break;
                }
                if (VERBOSE) {
                    Log.i(TAG, "audio encoder: returned input buffer: " + encoderInputBufferIndex);
                }
            ...
            while (mCopyVideo && !videoEncoderDone
                ...
                if ((videoEncoderOutputBufferInfo.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG)
                        != 0) {
                    if (VERBOSE) Log.i(TAG, "video encoder: codec config buffer");
                    // Simply ignore codec config buffers.
                    videoEncoder.releaseOutputBuffer(encoderOutputBufferIndex, false);
                    break;
                }
                if (VERBOSE) {
                    Log.i(TAG, "video encoder: returned buffer for time "
                ...
                if ((videoEncoderOutputBufferInfo.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM)
                        != 0) {
                    if (VERBOSE) Log.i(TAG, "video encoder: EOS");
            ...
            while (mCopyAudio && !audioEncoderDone
                ...
                if (encoderOutputBufferIndex == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    if (VERBOSE) Log.i(TAG, "no audio encoder output buffer");
                    break;
                }
                if (encoderOutputBufferIndex == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
                    if (VERBOSE) Log.i(TAG, "audio encoder: output buffers changed");
                    audioEncoderOutputBuffers = audioEncoder.getOutputBuffers();
                    break;
                }
                if (encoderOutputBufferIndex == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                ...
                assertTrue("should have added track before processing output", muxing);
                if (VERBOSE) {
                    Log.i(TAG, "audio encoder: returned output buffer: "
                            + encoderOutputBufferIndex);
                    Log.i(TAG, "audio encoder: returned buffer of size "
                ...
                if (VERBOSE) {
                    Log.i(TAG, "audio encoder: returned buffer for time "
